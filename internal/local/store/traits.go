...
type PersonalTrait struct {
	ID         int64     `db:"id"`
	Trait      string    `db:"trait"`      // 特征条目文本（原 item，建议改�?trait�?
	Category   int       `db:"category"`   // 特征分类 1~14
	Confidence int       `db:"confidence"` // 置信度（整数�?
	HalfLife   int       `db:"half_life"`  // 半衰�? 1-短期,2-中期,3-长期,4-永久
...
type TraitKeyword struct {
	ID       int64     `db:"id"`
	Word     string    `db:"word"`     // 关键词文�?
	Kind     int       `db:"kind"`     // 字母分类: 1=A,2=B,3=C,4=D,5=E,6=F（原 category，建议改�?kind�?
