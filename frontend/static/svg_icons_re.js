// ============================================================
// svg_icons_re.js — SVG 图标常量 ES Module 重新导出
//
// 此文件将 svg_icons.js（常规 <script>，注册到 window）中的
// 图标常量以 ES Module 方式重新导出，供其他 JS 模块使用
// import { ICON_XXX } from './svg_icons_re.js' 导入。
//
// 所有图标的实际值定义在 svg_icons.js 中，此文件仅做转发。
// ============================================================

export const ICON_COPY      = window.ICON_COPY;
export const ICON_SEND      = window.ICON_SEND;
export const ICON_DELETE    = window.ICON_DELETE;
export const ICON_GLOBE     = window.ICON_GLOBE;
export const ICON_MOON      = window.ICON_MOON;
export const ICON_SUN       = window.ICON_SUN;
export const ICON_TOGGLE_OPEN  = window.ICON_TOGGLE_OPEN;
export const ICON_TOGGLE_CLOSE = window.ICON_TOGGLE_CLOSE;
export const ICON_AI_TITLE  = window.ICON_AI_TITLE;
export const ICON_EDIT      = window.ICON_EDIT;
export const ICON_DOTS      = window.ICON_DOTS;
export const ICON_CLOSE     = window.ICON_CLOSE;
export const ICON_PIN       = window.ICON_PIN;
export const ICON_STOP      = window.ICON_STOP;
export const ICON_ATTACH    = window.ICON_ATTACH;
export const ICON_NEW_CHAT  = window.ICON_NEW_CHAT;
export const ICON_UNDO      = window.ICON_UNDO;
export const ICON_TRASH_RESTORE = window.ICON_TRASH_RESTORE;
export const ICON_COPY_MSG  = window.ICON_COPY_MSG;
export const ICON_LOADING    = window.ICON_LOADING;
export const ICON_DEEP_THINK = window.ICON_DEEP_THINK;
export const ICON_USER       = window.ICON_USER;
export const ICON_TRASH      = window.ICON_TRASH;
