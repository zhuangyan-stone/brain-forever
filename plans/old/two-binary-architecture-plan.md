...
# 构建 local-server
CGO_ENABLED=1 go build -o local-server ./cmd/server/
```
...
go build -o local-server ./cmd/server/
...
echo [3/4] Building local-server...
go build -o local-server.exe .\cmd\server\
...
| 2 | 创建 `cmd/server/main.go` | 前端静态文件服务 + `httputil.ReverseProxy` 代理 `/api/*` | 新文件 |
